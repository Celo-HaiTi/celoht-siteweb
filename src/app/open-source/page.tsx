...
export const metadata: Metadata = {
  title: "CeloHT Open Source | Official Public Documentation & Transparency Center",
  description:
  ...
  openGraph: {
    ...
    description: "Official CeloHT Open Source Center: public documentation, governance, and transparency.",
  ...
  return (
    <>
      ...
      <section className="mx-auto flex max-w-6xl flex-wrap gap-4 px-4 pb-10 sm:px-6">
        <Link href="#public-resources" className="inline-flex min-h-12 items-center gap-2 rounded-md bg-gold-500 px-6 py-3 text-sm font-bold text-navy-950 transition-colors hover:bg-gold-300">
          Explore Public Resources →
        </Link>
        <Link href="/docs" className="inline-flex min-h-12 items-center gap-2 rounded-md border border-parchment-100/30 px-6 py-3 text-sm font-semibold text-parchment-50 transition-colors hover:border-gold-300 hover:text-gold-300">
          Read the Documentation
        </Link>
      </section>

      {/* PRINCIPLES */}
      <Section eyebrow="Our Open Source Principles" title="Standards guiding our work">
        <div className="grid gap-px overflow-hidden border border-navy-700/15 bg-navy-700/15 sm:grid-cols-2 lg:grid-cols-3">
          {openSourcePrinciples.map((principle) => (
            <div key={principle.title} className="bg-parchment-50 p-6 dark:bg-navy-900">
              <p className="font-mono text-xs tracking-[0.16em] text-gold-800 dark:text-gold-300">{principle.title.toUpperCase()}</p>
              <p className="mt-5 text-sm leading-6 text-ink-soft dark:text-parchment-100/70">{principle.description}</p>
            </div>
          ))}
        </div>
      </Section>
        ...
        <Section eyebrow="What We Make Public" title="Nine areas of public transparency">
          <div className="grid gap-px overflow-hidden border border-parchment-100/15 bg-parchment-100/15 sm:grid-cols-2 lg:grid-cols-3">
            {whatWePublic.map((resource) => {
              ...
              return (
                <Link key={resource.title} href={resource.internalLink} className="group bg-navy-950 p-6 transition-colors hover:bg-navy-800">
                  <Icon size={21} className="text-gold-300" aria-hidden="true" />
                  <h3 className="mt-8 font-display text-xl font-semibold">{resource.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-parchment-100/65">{resource.description}</p>
                </Link>
              );
            })}
          </div>
      ...
      <Section id="projects" eyebrow="Projects & Public Infrastructure" title="The CeloHT ecosystem">
        <p className="mb-8 max-w-3xl text-base leading-7 text-ink-soft dark:text-parchment-100/70">
          CeloHT maintains a public ecosystem of code, documentation, research, and infrastructure. Each project serves a specific role and carries a status designation indicating its current state.
        </p>
        <div className="grid gap-4 sm:grid-cols-2">
          {publicProjects.map((project) => (
            <div key={project.name} className="flex flex-col rounded-2xl border border-navy-700/15 p-6 dark:border-parchment-100/10">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <h3 className="font-display text-lg font-semibold">{project.name}</h3>
                  <p className="mt-2 text-xs leading-5 text-ink-soft dark:text-parchment-100/70">{project.purpose}</p>
                </div>
                <span className={`whitespace-nowrap rounded-full border px-2.5 py-1 text-xs font-semibold ${getStatusColor(project.status)}`}>
              ...
              <div className="mt-4 space-y-2 border-t border-navy-700/15 pt-4 dark:border-parchment-100/10">
                ...
                <p className="text-sm text-ink-soft dark:text-parchment-100/70">{project.role}</p>
              </div>

              {project.technology && (
                <div className="mt-3 space-y-2">
                  <p className="text-sm font-semibold text-gold-500">Technology</p>
                  <p className="text-sm text-ink-soft dark:text-parchment-100/70">{project.technology}</p>
                </div>
              )}
            </div>
          ))}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <p className="mt-1 text-xs leading-5 text-ink-soft dark:text-parchment-100/70">{item.description}</p>
                  </div>
      ...
      <section className="border-y border-forest-500/20 bg-forest-700 text-parchment-50">
        <Section eyebrow="Transparency" title="How we maintain public accountability">
          <p className="mb-8 text-base leading-7">
            Transparency is not optional at CeloHT. We distinguish current work from planned work, maintain public documentation, and preserve a history of decisions and organizational development.
          </p>
          <div className="grid gap-4 sm:grid-cols-2">
            {transparencyAreas.map((area) => (
              ...
              >
                ...
                <p className="mt-2 text-sm leading-6 text-forest-50/90">{area.description}</p>
                {area.status && <p className="mt-3 text-xs font-semibold text-gold-300">{area.status}</p>}
              </Link>
            ))}
          </div>
        </Section>
      ...
      {/* SECURITY */}
      <Section eyebrow="Security" title="Public security practices and responsible disclosure">
        <div className="grid gap-8 lg:grid-cols-[1fr_1fr]">
          <p className="text-base leading-7 text-ink-soft dark:text-parchment-100/70">
            CeloHT documents its security practices and maintains a responsible disclosure process for vulnerability reporting. Security information is publicly available to enable community review and verification.
          </p>
          <div className="space-y-3">
            {securityItems.map((item) => (
              ...
              >
                <p className="font-semibold">{item.title}</p>
                <p className="mt-1 text-sm text-ink-soft dark:text-parchment-100/70">{item.description}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* RESEARCH */}
      <Section eyebrow="Research" title="Technical analysis and evidence-based work">
        <p className="mb-8 max-w-3xl text-base leading-7 text-ink-soft dark:text-parchment-100/70">
          CeloHT publishes research papers, technical specifications, RFCs, and evidence-based analysis. Research is organized and made accessible through public repositories and documentation.
        </p>
        <Link
          href="/research"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {policies.map((policy) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-ink-soft dark:text-parchment-100/70">{policy.description}</p>
              </Link>
            ))}
          </div>
        </Section>
      ...
      <Section eyebrow="Roadmap" title="Public development direction">
        <p className="mb-8 max-w-3xl text-base leading-7 text-ink-soft dark:text-parchment-100/70">
          CeloHT maintains a public roadmap distinguishing completed work, current work, near-term plans, and longer-term vision. This allows the community to understand our development direction and progress.
        </p>
        <div className="grid gap-6 lg:grid-cols-2">
          {roadmapPhases.map((phase) => (
            <div key={phase.title} className="rounded-2xl border border-navy-700/15 p-6 dark:border-parchment-100/10">
              ...
              <ul className="mt-4 space-y-2">
                {phase.items.map((item, idx) => (
                  <li key={idx} className="flex gap-3 text-sm text-ink-soft dark:text-parchment-100/70">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {brandResources.map((resource) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-ink-soft dark:text-parchment-100/70">{resource.description}</p>
            </Link>
          ))}
        </div>
      </Section>
      ...
      <section className="border-y border-navy-700/10 bg-navy-950 text-parchment-50 dark:border-parchment-100/10">
        <Section id="public-resources" eyebrow="Public Resources Directory" title="Where to find information">
          <p className="mb-8 text-base leading-7 text-parchment-100/70">
            Complete directory of public CeloHT information. All resources are internal to the CeloHT website and do not require external navigation.
          </p>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {publicResourceDirectory.map((category) => {
              ...
              return (
                <div key={category.title} className="rounded-2xl border border-parchment-100/15 bg-navy-900 p-6">
                  <Icon size={24} className="text-gold-300" aria-hidden="true" />
                  <h3 className="mt-4 font-display text-lg font-semibold">{category.title}</h3>
                  <ul className="mt-4 space-y-2">
                    {category.items.map((item) => (
                      <li key={item.label}>
                        <Link href={item.href} className="text-sm text-gold-300 hover:text-gold-100 underline-offset-4 hover:underline">
                          {item.label}
                        </Link>
                      </li>
                    ))}
