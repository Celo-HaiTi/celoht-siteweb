  ...
  return (
    <>
      ...
      <Section eyebrow="Operating standards" title="How agents work">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-navy-700/15 p-5 dark:border-parchment-100/10">
            ...
            <p className="mt-2 text-sm text-ink-soft dark:text-parchment-100/70">
              A lightweight process adapted to local context, reducing fraud without creating
        ...
        <ol className="max-w-2xl space-y-3">
          {steps.map((step, index) => (
            <li key={step} className="flex items-start gap-4">
              ...
              <span className="pt-1 text-sm text-ink-soft dark:text-parchment-100/70">{step}</span>
            </li>
          ))}
        </ol>
      </Section>

      <Section eyebrow="Connection to Reforestation" title="Many agents wear two hats">
        <p className="max-w-3xl text-ink-soft dark:text-parchment-100/75">
          Some community agents also take on responsibility for coordinating local reforestation
          activity, creating a direct operational link between financial inclusion and environmental
          sustainability.
        </p>
      </Section>
      ...
      <CTASection
        ...
        secondary={{ label: "See the reforestation link", href: "/reforestation" }}
      />
    </>
  );
}
