...
export function PageHero({ eyebrow, title, lead, tone = "navy" }: Props) {
  return (
    <div className="relative overflow-hidden border-b border-navy-800 bg-navy-950 text-parchment-50">
      <div className="celoht-grid pointer-events-none absolute inset-0 opacity-60" aria-hidden="true" />
      <div className="pointer-events-none absolute -right-40 -top-40 h-96 w-96 rounded-full bg-gold-500/10 blur-3xl" aria-hidden="true" />
      <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-12 sm:px-6 lg:pb-24 lg:pt-20">
        ...
        <p className="mt-7 max-w-2xl text-lg leading-8 text-parchment-100/75 sm:text-xl">{lead}</p>
      </div>
    </div>
  );
}
