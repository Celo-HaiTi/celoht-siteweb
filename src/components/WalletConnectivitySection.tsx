  ...
  return (
    ...
    >
      ...
      <p className="mt-4 max-w-3xl text-sm leading-7 text-ink-soft dark:text-parchment-100/65">
        CeloHT is designed to make digital finance accessible across the Celo
        ecosystem. Our wallet infrastructure is built to support seamless
        connections between users, CeloHT applications, and modern Web3 wallets.
      </p>

      <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-gold-500/20 bg-[linear-gradient(110deg,rgba(255,212,59,0.08),rgba(10,27,48,0.72))] p-5 shadow-[0_16px_44px_rgba(2,8,20,0.16)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        {walletCards.map((card) => {
          ...
          return (
            <article
              key={card.title}
              className="group relative flex h-full min-h-[34rem] flex-col overflow-hidden rounded-2xl border border-parchment-100/12 bg-[linear-gradient(145deg,rgba(16,38,64,0.92),rgba(6,20,38,0.98))] p-6 shadow-[0_20px_60px_rgba(2,8,20,0.2)] transition-all duration-300 hover:-translate-y-1 hover:border-gold-500/45 hover:shadow-[0_24px_70px_rgba(2,8,20,0.35)] sm:p-8"
            >
              <div
                className="pointer-events-none absolute -right-12 -top-12 h-36 w-36 rounded-full bg-gold-500/8 blur-3xl transition-opacity duration-300 group-hover:opacity-100"
                aria-hidden="true"
              />
              <div className="flex items-start justify-between gap-4">
                ...
                <Icon
                  className="mt-1 shrink-0 text-gold-300/80"
                  size={22}
                  aria-hidden="true"
                />
              </div>
              <p className="relative mt-7 text-sm leading-7 text-parchment-100/70">
                {card.description}
              </p>
              <ul className="relative mt-6 grid gap-3 border-t border-parchment-100/10 pt-6 text-sm text-parchment-100/80 sm:grid-cols-2">
      ...
      <div className="mt-10 rounded-2xl border border-gold-500/20 bg-navy-900/65 px-5 py-7 sm:px-8">
        ...
        <div className="mt-7 grid items-center gap-3 text-center sm:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr]">
          <div className="font-display text-lg font-semibold text-parchment-50">
            CeloHT
          </div>
          <span className="hidden text-gold-300/70 sm:block" aria-hidden="true">
            -&gt;
          </span>
          <div className="flex flex-wrap justify-center gap-2 text-sm text-parchment-100/75">
            <span className="rounded-full border border-gold-500/25 bg-gold-500/5 px-3 py-1.5">
              Valora
            </span>
